...
import { getAuthHeader } from '../_utils/AuthHeaders.js'
  ...
  // --- ➕ Create a new branch ---
  export const createType = async (propertyTypeCode, propertyTypeName, propertyTypePoundage) => {  
    ...
    if (!res.ok) throw new Error(data.message || 'Failed to create property type.');
    return {
      status: res.status,
      data
    };
  ...
  export const getTypes = async () => {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || 'Failed to fetch property types.');
    return data;
  };
  ...
  export const updateType = async (typeId, fieldName, newValue) => {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || 'Failed to update property type.');
    return data;
  };
  
  export const getType = async (typeId) => {
    if (!typeId || isNaN(typeId)) {
      throw new Error('Validation Error: Invalid property type ID.');
    }
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || 'Failed to fetch property type.');
    return data;
  };
