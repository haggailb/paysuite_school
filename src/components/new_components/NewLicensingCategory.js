...
import { getCOA, getBusinessCategories } from "../../_services/dataServices";
import Select from "react-select";

const NewLicensingCategory = () => {
  ...
  const [newMarket, setNewMarket] = useState({
    ...
    useEffect(() => {
      setCOA(getCOA());
      setCategory(getBusinessCategories());
    }, []);
  
  const [coAccounts, setCOA] = useState([]);
  const [categorries, setCategory] = useState([]);
...
export default NewLicensingCategory;
