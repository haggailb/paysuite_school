...
import { sampleSystemModules, sampleAccessLevelModules } from '../_services/dataServices';
...
const AccessLevels = () => {
  ...
  const filteredList = sampleAccessLevelModules.filter(listItem => {
      ...
      <Row>
        ...
        <Col md={4} className='mb-3'>
          <Card className="h-100">
            <Card.Body>
              <h5>Module Allocation</h5>
              <Form onSubmit={handleSubmit} noValidate validated={validated}>
                <Form.Group className="mb-3">
                  <Form.Label className='required'>User Level</Form.Label>
                  <Form.Control 
                    required
                    name='userLevel' 
                    value={formData.userLevel} 
                    onChange={handleChange}
                    placeholder='Enter New or Existing Level' 
                  />
                  <Form.Control.Feedback type="invalid"> Invalid Program Code. </Form.Control.Feedback>
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label className='required'>System Module</Form.Label>
                  <Select
                    required
                    name="moduleId"
                    options={sampleSystemModules}
                    getOptionLabel={(e) => e.moduleName }
                    getOptionValue={(e) => e.moduleId}
                    onChange={(selectedOption) => handleSelectChange("moduleId", selectedOption.moduleId)}
                    placeholder="-- Select Module --"
                  />
                  <Form.Control.Feedback type="invalid"> Select a system module. </Form.Control.Feedback>
                </Form.Group>
              ...
              <div className="custom-table-wrapper">
                <Table striped bordered hover className="custom-table" >
                  ...
                  <tbody>
                    ...
                    {filteredList.map((levelModule, index) => (
                      <tr key={index}>
                        <td>{levelModule.levelName}</td>
                        <td>{levelModule.moduleName}</td>
                        <td>
                          <Dropdown as={ButtonGroup}>
