...
const Retentions = () => {
  ...
  useEffect(() => {
    setPageTitle('Student Retentions');
    setBackUrl('/');
  }, []);
  ...
  const handleRetention = (student) => {
    navigate(`/retentions/${student.studentId}`, { state: { student } });
  };
          ...
          <tbody>
            ...
            {paginatedList.map((student, index) => (
              <tr key={index}>
                ...
                <td>
                  <Button className=" btn" onClick={() => handleRetention(student)}>
                    Evaluate
                  </Button>
                </td>
              </tr>
