...
import { getInstitution, getCOA, getCoaCategories } from "../../_services/dataServices";
import NewCoaCategory from "../../components/new_components/NewCoaCategory";
...
const CoaCategories = () => {
  ...
  const [newCoaCategory, setNewCoaCategory] = useState({
    locId: "",
    zoneId: "",
    code: "",
    name: "",
    ...
    useEffect(() => {
      setCOA(getCOA());
      setCoaCategory(getCoaCategories());
    }, []);
  
  const [coAccounts, setCOA] = useState([]);
    const institution = getInstitution;
  const [coaCategories, setCoaCategory] = useState([]);

  const [showViewModal, setShowViewModal] = useState(false);
  const [showEditModal, setShowEditModal] = useState(false);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [selectedCoaCategory, seSelectedCoaCategory] = useState(null);

  const handleOpenModal = (bank, modalType) => {
    seSelectedCoaCategory(bank);
  ...
  const filterCoaCategories = coaCategories.filter(coaCategory =>
    Object.values(coaCategory).some(value =>
      value.toString().toLowerCase().includes(search.toLowerCase())
    )
  );

  const totalPages = Math.ceil(filterCoaCategories.length / recordsPerPage);
  const paginatedCoaCategories = filterCoaCategories.slice(
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `${institution.name} Registered COA Categories.csv`);
  ...
  const handleExportXLSX = () => {
    const worksheet = XLSX.utils.json_to_sheet(filterCoaCategories);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "COA Categories");
    XLSX.writeFile(workbook, `${institution.name} Registered COA Categories.xlsx`);
  };

  const handleSelectChange = (field, selectedOption) => {
    setNewCoaCategory({ ...newCoaCategory, [field]: selectedOption });
  };
  const handleChange = (e) => {
    const { name, value } = e.target;
    setNewCoaCategory({ ...newCoaCategory, [name]: value });
  };
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(`
            <html>
            <head>
                <title>${institution.name} Registered COA Categories</title>
                <style>
            ...
            <body>
                ...
                <h3>Registered COA Categories</h3>
  ...
  return (
    <div className="form-container mb-5">
      <h2 className="page-title text-center mb-4"> Fee Structure <br></br> ( Chart of Accounts )</h2>
      <Row className="mb-3">
      ...
      <Table striped bordered hover className="data-table">
        ...
        <tbody>
          {paginatedCoaCategories.map((coaCategory, index) => (
            <tr key={index}>
              <td>{coaCategory.code}</td>
              <td>{coaCategory.name}</td>
              <td>{coaCategory.category}</td>
      ...
      <Modal show={showViewModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        ...
        <Modal.Body>
          {selectedCoaCategory && (
            <div>
              <p><strong>Account Code:</strong> {selectedCoaCategory.code}</p>
              <p><strong>Account Name:</strong> {selectedCoaCategory.name}</p>
              <p><strong>Category:</strong> {selectedCoaCategory.category}</p>
              <p><strong>Amount:</strong> {formatCurrency(selectedCoaCategory.amount, "ZMW")}</p>
            </div>
          )}
        </Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={showEditModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        ...
        <Modal.Body>
          {selectedCoaCategory && (
            ...
            <Row>
              <Col md={6}>
                <p><strong>Account Code:</strong> <br></br> {selectedCoaCategory.code}</p>
              </Col> 
              <Col md={6}>
            ...
            <Row>
              <Col md={6}>
                <p><strong>Category:</strong><br></br> {selectedCoaCategory.category}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
            ...
            <Row>
              <Col md={6}>
                <p><strong>Amount:</strong> <br></br> {formatCurrency(selectedCoaCategory.amount, "ZMW")}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
        ...
        <Modal.Body className="text-center">
          {selectedCoaCategory && (
            <Card>
              ...
              <Card.Body>
                <Form>
                    <h2>{selectedCoaCategory.code}, {selectedCoaCategory.name}<br></br> {selectedCoaCategory.category} </h2>
                </Form>
              </Card.Body>
              <Card.Footer>
                <Row className="">
