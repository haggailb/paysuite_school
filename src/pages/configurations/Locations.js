...
import NewLocation from "../../components/new_components/NewLocation";
import { updateLocation, getLocations } from "../../_services/locationServices";
...
const Locations = () => {
  ...
  const handleSelectChange = async (field, newValue) =>  {
    ...
    setLoading(true);
    try {
      const result = await updateLocation(selectedLocation.locationId, field, newValue);
      showMessageModal({
        heading: 'Success!',
      ...
      showMessageModal({
        heading: 'Updating location failed!',
  ...
  const handleFieldUpdate = async (e, locationId, fieldName, newValue) => {
    e.preventDefault();
    setLoading(true);
      try {
        const result = await updateLocation(locationId, fieldName, newValue);
        showMessageModal({
          heading: 'Success!',
          message: `Location update successfully`,
          messageType: 'success',
  ...
  const onFieldKeyDown = (fieldName) => (e) => {
    if (e.key === 'Enter') {
      handleFieldUpdate(e, selectedLocation.locationId, fieldName, e.target.value);
    }
  };
  ...
  const handleExportCSV = () => {
    const csvContent =
      ...
      filteredLocations
        .map(
          (type) =>
            `${type.locationCode},${type.locationName},${type.zoneName}`
        )
        .join("\n");
    const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.setAttribute("download", `${institution.name} Registered Locations.csv`);
  ...
  const handleExportXLSX = () => {
    const worksheet = XLSX.utils.json_to_sheet(filteredLocations);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Locations");
    XLSX.writeFile(workbook, `${institution.name} Registered Locations.xlsx`);
  };
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(`
            ...
            <body>
                ...
                <h3 style="margin: 0">Registered Locations</h3>
  ...
  return (
    <div className="form-container mb-5">
      <h2 className="page-title text-center mb-4">Locations</h2>
      <Row className="mb-3">
        ...
        <tbody>
          {paginatedlocations.map((location, index) => (
            <tr key={index}>
              ...
              <td>{location.zoneName}</td>
              <td>
                <Button variant="outline-primary" size="sm" className="me-2" onClick={() => handleOpenModal(location, "view")}><FaEye /></Button>
                <Button variant="outline-success" size="sm" className="me-2" onClick={() => handleOpenModal(location, "edit")}><FaEdit /></Button>
                {/* <Button variant="outline-danger" size="sm" className="me-2" onClick={() => handleOpenModal(location, "delete")}><FaBan /></Button> */}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <Modal show={showNewModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        <Modal.Header closeButton className="bg-primary">
          <Modal.Title className="text-center">Register New Location</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            <NewLocation />
        </Modal.Body>
        ...
        <Modal.Body>
          {selectedLocation && (
            <div>
              ...
              <p><strong>Location Code:</strong> {selectedLocation.locationCode}</p>
              <p><strong>Location Name:</strong> {selectedLocation.locationName}</p>
            </div>
          )}
        </Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={showEditModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        ...
        <Modal.Body>
          {selectedLocation && (
            ...
            <Row>
              <Col md={6}>
                <p><strong>Code:</strong><br></br> {selectedLocation.locationCode}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
                  <Form.Control 
                    ...
                    defaultValue={selectedLocation.locationCode}
                    onKeyDown={onFieldKeyDown('locationCode')}
            ...
            <Row>
              <Col md={6}>
                <p><strong>Name:</strong> <br></br> {selectedLocation.locationName}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
                  <Form.Control 
            ...
            <Row>
              <Col md={6}>
                <p><strong>Zone:</strong> <br></br> {selectedLocation.zoneName}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
                  <Select
                    ...
                    onChange={(selectedOption) => handleSelectChange("zoneId", selectedOption.zoneId)}
      ...
      <Modal show={showDeleteModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        <Modal.Header closeButton className="bg-danger">
          <Modal.Title className="text-white text-center">Delete Location</Modal.Title>
        </Modal.Header>
        <Modal.Body className="text-center">
          {selectedLocation && (
            <Form>
                <p><strong>Are you sure you want to remove this location?</strong></p>
                <h2>{selectedLocation.code}, {selectedLocation.name}</h2>
                <Row className="mt-5">
                <Col md={6}>
