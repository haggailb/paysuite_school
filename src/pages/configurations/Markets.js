...
import NewMarket from "../../components/new_components/NewMarket";
import { updateMarket, getMarkets } from "../../_services/marketServices";
...
const Markets = () => {
  ...
  const [selectedMarket, setSelectedMarkets] = useState(null);

  const handleOpenModal = (bank, modalType) => {
    setSelectedMarkets(bank);
  ...
  const handleFieldUpdate = async (e, marketId, fieldName, newValue) => {
    e.preventDefault();
    setLoading(true);
      try {
        const result = await updateMarket(marketId, fieldName, newValue);
        showMessageModal({
          heading: 'Success!',
        ...
        showMessageModal({
          heading: 'Updating market failed!',
  ...
  const onFieldKeyDown = (fieldName) => (e) => {
    if (e.key === 'Enter') {
      handleFieldUpdate(e, selectedMarket.marketId, fieldName, e.target.value);
    }
  };
  ...
  const totalPages = Math.ceil(filteredMarkets.length / recordsPerPage);
  ...
  const handleExportCSV = () => {
    const csvContent =
      "Market Name, Location, Zone\n" +
      filteredMarkets
        .map(
          (market) =>
            `${market.marketName},${market.locationName},${market.zoneName}`
        )
        .join("\n");
    const blob = new Blob([csvContent], { market: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.setAttribute("download", `${institution.name} Registered Markets.csv`);
  ...
  const handleExportXLSX = () => {
    const worksheet = XLSX.utils.json_to_sheet(filteredMarkets);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Markets");
    XLSX.writeFile(workbook, `${institution.name} Registered Markets.xlsx`);
  };
    ...
    const handlePrint = () => {
        ...
        printWindow.document.write(`
            ...
            <body>
                ...
                <h3 style="margin: 0">Registered Markets</h3>
                    ...
                    <tbody>
                        ${filteredMarkets
                            ...
                            <tr>
                                ...
                                <td>${market.locationName}</td>
                                <td>${market.zoneName}</td>
                            </tr>`
                            )
                            .join("")}
                    </tbody>
  ...
  return (
    <div className="form-container mb-5">
      <h2 className="page-title text-center mb-4">Markets</h2>
      <Row className="mb-3">
        ...
        <tbody>
          {paginatedMarkets.map((market, index) => (
            <tr key={index}>
              ...
              <td>{market.zoneName}</td>
              <td>
                <Button variant="outline-primary" size="sm" className="me-2" onClick={() => handleOpenModal(market, "view")}><FaEye /></Button>
                <Button variant="outline-success" size="sm" className="me-2" onClick={() => handleOpenModal(market, "edit")}><FaEdit /></Button>
                {/* <Button variant="outline-danger" size="sm" className="me-2" onClick={() => handleOpenModal(market, "delete")}><FaBan /></Button> */}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <Modal show={showNewModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        <Modal.Header closeButton className="bg-primary">
          <Modal.Title className="text-center">Register New Market</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <NewMarket />
        </Modal.Body>
        ...
        <Modal.Body>
          {selectedMarket && (
            <div>
              ...
              <p><strong>Market Name:</strong> {selectedMarket.marketName}</p>
              <p><strong>Location:</strong> {selectedMarket.locationName}</p>
              <p><strong>Zone:</strong> {selectedMarket.zoneName}</p>
            </div>
          )}
        </Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={showEditModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        ...
        <Modal.Body>
          {selectedMarket && (
            ...
            <Row>
              <Col md={6}>
                <p><strong>Market Name:</strong><br></br> {selectedMarket.marketName}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
                  <Form.Control 
                    ...
                    onKeyDown={onFieldKeyDown("marketName")}
                  />
                </Form.Group>
              </Col> 
            </Row>
            <Row>
              <Col md={6}>
                <p><strong>Location:</strong> <br></br> {selectedMarket.locationName}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
                  <Form.Control 
                    ...
                    defaultValue={selectedMarket.locationName}
                    onKeyDown={onFieldKeyDown("locationName")}
                    disabled
                  />
                </Form.Group>
              </Col> 
            ...
            <Row>
              <Col md={6}>
                <p><strong>Zone:</strong> <br></br> {selectedMarket.zoneName}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
                  <Form.Control 
                    ...
                    defaultValue={selectedMarket.zoneName}
      ...
      <Modal show={showDeleteModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        <Modal.Header closeButton className="bg-danger">
          <Modal.Title className="text-white text-center">Delete Market</Modal.Title>
        </Modal.Header>
        <Modal.Body className="text-center">
          {selectedMarket && (
            <Form>
                <p><strong>Are you sure you want to remove this market?</strong></p>
                <h2>{selectedMarket.code}, {selectedMarket.name}</h2>
                <Row className="mt-5">
                <Col md={6}>
