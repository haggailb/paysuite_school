...
import { getBankAccounts, updateBankAccountField } from "../../_services/bankAccountServices";
...
const BankAccounts = () => {
  ...
  const totalPages = Math.ceil(filteredBankAccounts.length / recordsPerPage);
  ...
  const handleExportXLSX = () => {
    const worksheet = XLSX.utils.json_to_sheet(filteredBankAccounts);
  ...
  const handleSelectChange = (field, selectedOption) => {
    setNewBankAccount({ ...newBankAccount, [field]: selectedOption });
  };
  ...
  return (
    <div className="form-container mb-5">
      <h2 className="page-title text-center mb-4">Bank Accounts</h2>
      <Row className="mb-3">
        ...
        <tbody>
          {paginatedBankAccounts.length === 0 && (
            <tr className="text-center">
              <td colSpan="6" className="text-danger">No accounts found</td>
            </tr>
          )}
          {paginatedBankAccounts.map((account, index) => (
            <tr key={index}>
              <td>{account.bankName}</td>
              <td>{account.branchName}</td>
              <td>{account.accountNumber}</td>
              <td>{account.accountName}</td>
              <td>{formatCurrency(account.balanceEstimate || 0, "ZMW", true)}</td>
              <td>
