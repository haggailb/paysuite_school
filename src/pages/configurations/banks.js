...
import NewBank from "../../components/new_components/NewBank";
import { updateBankField, getAllBanks } from '../../_services/bankServices';
...
const Banks = () => {
  ...
  const [banksList, setBanks] = useState([]);
  ...
  const handleFieldUpdate = async (e, bankId, fieldName, newValue) => {
    e.preventDefault();
    setLoading(true);
      try {
        const result = await updateBankField(bankId, fieldName, newValue);
  ...
  const onFieldKeyDown = (fieldName) => (e) => {
    if (e.key === 'Enter') {
      handleFieldUpdate(e, selectedBank.bankId, fieldName, e.target.value);
    }
  };
  ...
  const handleExportCSV = () => {
    const csvContent =
      ...
      filteredBanks
        .map(
          (bank) =>
            `${bank.bankId},${bank.bankName},${bank.bankCodeName}`
        )
        .join("\n");
  ...
  return (
    <div className="form-container mb-5">
      <h2 className="page-title text-center mb-4">Configure Banks</h2>
      <Row className="mb-3">
        ...
        <tbody>
          {paginatedBanks.length === 0 && (
            <tr className="text-center">
              <td colSpan="4" className="text-danger">No banks found</td>
            </tr>
          )}
          {paginatedBanks.map((bank, index) => (
            <tr key={index}>
              ...
              <td>{bank.bankCodeName}</td>
              <td>
                <Button variant="outline-primary" size="sm" className="me-2" onClick={() => handleOpenModal(bank, "view")}><FaEye /></Button>
                <Button variant="outline-success" size="sm" className="me-2" onClick={() => handleOpenModal(bank, "edit")}><FaEdit /></Button>
      ...
      <Modal show={showNewModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        <Modal.Header closeButton className="bg-primary">
          <Modal.Title className="text-center">Register New Bank</Modal.Title>
        </Modal.Header>
        <Modal.Body>
            <NewBank />
        </Modal.Body>
      ...
      <Modal show={showViewModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        ...
        <Modal.Body>
          {selectedBank && (
            <div>
              <p><strong>Bank ID:</strong> {selectedBank.bankId}</p>
              <p><strong>Bank Name:</strong> {selectedBank.bankName}</p>
              <p><strong>Bank Code Name:</strong> {selectedBank.bankCodeName}</p>
            </div>
          )}
        </Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={showEditModal} onHide={handleCloseModal} backdrop="static" keyboard={false}>
        ...
        <Modal.Body>
          {selectedBank && (
            ...
            <Row>
              <Col md={6}>
                <p><strong>Bank Name:</strong><br></br> {selectedBank.bankName}</p>
              </Col> 
              <Col md={6}>
            ...
            <Row>
              <Col md={6}>
                <p><strong>Bank Code Name:</strong><br></br>{selectedBank.bankCodeName}</p>
              </Col> 
              <Col md={6}>
                <Form.Group className="my-3">
                  <Form.Control
                    ...
                    onKeyDown={onFieldKeyDown('bankCodeName')}
        ...
        <Modal.Body className="text-center">
          {selectedBank && (
            <Form>
                ...
                <h2>{selectedBank.bankName}, {selectedBank.bankCodeName}</h2>
                <Row className="mt-5">
                <Col md={6}>
