...
import { getCOA, getLicencingCategories, getBusinesses } from "../../_services/dataServices";
  
  const today = <CurleaseDate />;

const InvoiceLicencing = () => {
  ...
  useEffect(() => {
    setCOA(getCOA());
    setLicenceCategories(getLicencingCategories());
  ...
  const [liceingStructures, setLicenceCategories] = useState([]);
  ...
  return (
    ...
    <Container className="flex-column justify-content-center align-items-center bg-light">
      <h2 className="page-title text-center mb-4"> New Invoice <br></br>( Busincess Licencing)</h2>
                ...
                <Row>
                  <Col md={6}>
                    <Form.Group className="mb-2">
                      <Form.Label>Busincess Code</Form.Label>
                      <Select
                        required
                        options={businesses}
                        getOptionLabel={(e) => `${e.businessId} - ${e.name}`}
            ...
            <Card className="h-100 mb-3 shadow">
              ...
              <Card.Body>
                <Form.Group className="mb-2">
                  ...
                  <Select
                    required
                    options={liceingStructures}
                    getOptionLabel={(e) => `${e.category} - ${e.name}`}
...
export default InvoiceLicencing;
